...
void Viewer::ThreadLoop() {
    pangolin::CreateWindowAndBind("Demoam", 1024, 768);
...
cv::Mat Viewer::PlotFrameImage() {
    cv::Mat img_out;
    cv::cvtColor(current_frame_ -> img_left_, img_out, cv::COLOR_GRAY2BGR);
...
void Viewer::DrawMapPoints() {
    ...
    glPointSize(2);
    glBegin(GL_POINTS);
    for (auto& mappoint : active_mappoints_) {
        auto pos = mappoint.second->Pos();
